...
def get_categories_to_create_item_by_agent_without_all_estabs(agent, item_table_compound_id):
    return ItemCategory.objects.filter(item_table__company__in=Company.objects.filter(establishment__in=agent.establishments.all()), item_table__item_table_compound_id=item_table_compound_id).order_by('category_code')
